...
async def monitor_spp(bot_app):
    ...
    for nm_id in nm_ids:
        try:
            ...
            save_spp_history(
                ...
                current_price=data['site_price'],
                old_price=data['api_price'],
                spp_percent=data['spp_percent']
            )
            last = get_last_spp(nm_id)
            if not last:
                logger.info(f"📝 Первая проверка para {nm_id}")
            ...
            logger.error(f"❌ Ошибка мониторинга para {nm_id}: {e}")
            await asyncio.sleep(10)
    ...
    for brand in brands:
        try:
            ...
            if not nm_ids_brand:
                logger.warning(f"⚠️ Нет артикулов с nm_id para бренда {brand}")
                continue
            spp_values = []
            for nm_id in nm_ids_brand:
                ...
                await asyncio.sleep(3)
            if not spp_values:
                logger.warning(f"⚠️ Не удалось получить СПП para артикулов бренда {brand}")
                continue
            avg_spp = sum(spp_values) / len(spp_values)
            save_brand_history(brand, avg_spp)
            last = get_last_brand_spp(brand)
            if not last:
                logger.info(f"📝 Первая проверка средней СПП para бренда {brand}")
                continue
...
def generate_spp_graph(nm_id: int, width: int = 800, height: int = 400) -> Optional[str]:
    ...
    ax1.set_title(f'Динамика СПП para артикула {nm_id}')
