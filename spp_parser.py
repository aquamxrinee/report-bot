...
# Кеш для временной блокировки артикулов
_failed_cache = {}


def get_spp_for_article(nm_id: int, retries: int = 3) -> Optional[Dict[str, Any]]:
    """
    Получает данные о товаре с повторными попытками при ошибках.
    """
    # Если артикул недавно давал ошибку, не пытаемся снова
    ...
    for attempt in range(1, retries + 1):
        try:
            ...
            # Метод 2: селекторы, если JSON не сработал
            if not current_price:
                ...
                if price_el:
                    ...
                    # Метод 3: поиск цены в любом месте
            ...
            # Убираем артикул из кеша ошибок, если он там был
